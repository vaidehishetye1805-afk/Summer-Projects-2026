...
void solve()
{
    ...
    for(int t=1;t<=n;t++)
    {
        ...
        while(!(idx1==n && idx2==n))
        {
            idx1=lower_bound(wins1.begin()+last_idx+1,wins1.end(),t+win1)-wins1.begin();
            idx2=lower_bound(wins2.begin()+last_idx+1,wins2.end(),t+win2)-wins2.begin();
            //cout<<t<<" "<<last_idx<<" "<<idx1<<" "<<idx2<<endl;
            if(idx1<idx2)
            {
                setWins1++;
                win1=wins1[idx1];
        ...
        if(valid)
        {
        ans.push_back(make_pair(max(setWins1,setWins2),t));
        }
    }

...
void solve()
{
    ...
    double high=1e9;
    double candidate=0;
    vector<pair<double,double>> worst_case(n-1);
    double eps=1e-7;
    while(high-low>eps)
    {
        double mid=low+(high-low)/2;
        //cout<<mid<<endl;
        double minimum=mid*v[0]+x[0];
        for(int i=0;i<n-1;i++)
        {
            minimum=min(minimum,mid*v[i]+x[i]);
            worst_case[i].first=minimum;
        }
        double maximum=-mid*v[n-1]+x[n-1];
        for(int i=n-2;i>=0;i--)
        {
            maximum=max(maximum,-mid*v[i+1]+x[i+1]);
            worst_case[i].second=maximum;
        }
        bool possible=false;
        for(int i=0;i<n-1;i++)
        {
            //cout<<worst_case[i].first<<" "<<worst_case[i].second<<" "<<x[i]<<" "<<v[i]<<" "<<endl;
            if(worst_case[i].first>=worst_case[i].second && worst_case[i].first>=x[i] && worst_case[i].second<=x[i+1])
            possible=true;
        }
        if(possible)
        {
    ...
    cout << fixed << setprecision(16) << candidate << endl;
}

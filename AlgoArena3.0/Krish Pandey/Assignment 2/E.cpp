...
void solve()
{
    ...
    vector<vector<ll>> dp(z.size(),vector<ll>(2,0));
    dp[0][1]=z[0].first*z[0].second;
    dp[0][0]=0;
    for(int i=1;i<z.size();i++)
    {
        if(z[i].first==z[i-1].first+1)
        {
            dp[i][1]=dp[i-1][0]+z[i].first*z[i].second;
        }
        else
        {
            dp[i][1]=max(dp[i-1][0],dp[i-1][1])+z[i].first*z[i].second;
        }
        dp[i][0]=max(dp[i-1][0],dp[i-1][1]);
    }
    cout<<max(dp[z.size()-1][0],dp[z.size()-1][1])<<endl;
}

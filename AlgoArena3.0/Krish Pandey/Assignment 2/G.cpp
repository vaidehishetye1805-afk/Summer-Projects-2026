...
vector<vector<ll>> comp(int start_row, int start_col, vector<vector<ll>>& grid)
{
    int n = grid.size();
    int m = grid[0].size();
    int opp_row = n - 1 - start_row;
    int opp_col = m - 1 - start_col;
    int dx, dy;
    if (start_row > opp_row)
        dx = -1;
    else
        dx = 1;

    if (start_col > opp_col)
        dy = -1;
    else
        dy = 1;
    ...
    for (int r = start_row; r < n && r >= 0; r += dx)
    {
        for (int c = start_col; c < m && c >= 0; c += dy)
        {
            ...
            ll val1 = -INF;
            ll val2 = -INF;
            if (new_r1 >= 0 && new_r1 < n && new_c1 >= 0 && new_c1 < m)
            {
                val1 = dp[new_r1][new_c1] + grid[r][c];
            }
            int new_r2 = r;
            int new_c2 = c - dy;
            if (new_r2 >= 0 && new_r2 < n && new_c2 >= 0 && new_c2 < m)
            {
                val2 = dp[new_r2][new_c2] + grid[r][c];
            }
...
void solve()
{
    ...
    vector<vector<ll>> dp_start_1 = comp(0, 0, grid);
    vector<vector<ll>> dp_start_2 = comp(n - 1, 0, grid);
    vector<vector<ll>> dp_end_1 = comp(n - 1, m - 1, grid);
    vector<vector<ll>> dp_end_2 = comp(0, m - 1, grid);
    ...
    for (int i = 1; i < n - 1; i++)
    {
        for (int j = 1; j < m - 1; j++)
        {
            ll ans1 = dp_start_1[i - 1][j] + dp_end_1[i + 1][j] + dp_start_2[i][j - 1] + dp_end_2[i][j + 1];
            ll ans2 = dp_start_1[i][j - 1] + dp_end_1[i][j + 1] + dp_start_2[i + 1][j] + dp_end_2[i - 1][j];

...
void solve()
{
    ...
    vector<vector<int>> dp(k+1,vector<int>(28*200+1,-1));
    dp[0][0]=0;
    for(int i=0;i<n;i++)
    {
        int exp1=a[i];
        int exp2=b[i];
        for(int ct=k;ct>0;ct--)
        {
            for(int x=28*200;x>=exp2;x--)
            {
                if(dp[ct-1][x-exp2]!=-1)
                dp[ct][x]=max(dp[ct][x],dp[ct-1][x-exp2]+exp1);
            }
        }
    }
    int roundness=0;
    for(int x=0;x<=28*200;x++)
    {
        int exp2=x;
        int exp1=dp[k][x];
        roundness=max(roundness,min(exp1,exp2));
    }
    cout<<roundness<<endl;
